...
exports.index = function(req, res) {
	Event.find({type: req.params.type}).where('hidden').exists(false).sort('-date').limit(12).exec(function(err, events) {
...
exports.get_events = function(req, res) {
	...
	Query.where('hidden').exists(false).sort('-date').skip(post.skip).limit(post.limit).exec(function(err, events) {
